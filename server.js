...
const PENDING_FILE = process.env.PENDING_UTMIFY_FILE
  || path.join(isVercel ? "/tmp" : path.join(__dirname, "data"), "pending-utmify-orders.json");
...
app.post("/api/pix/create", async (req, res) => {
  try {
    ...
    debugLog("=== DEBUG /api/pix/create ===");
    debugLog("Body normalizado:", normalizedData);
    debugLog("Campos faltando:", missingFields);
    ...
    debugLog("Payload normalizado:", normalizedData);
    debugLog("Items enviados:", items);
    debugLog("Soma dos items:", totalItems);

    const client = brutalClient();
    const response = await client.post("/v1/payment-transaction/create", payload);
    debugLog("Status BrutalCash:", response.status);

    const parsed = extractPixData(response.data);

    if (!parsed.pixCode && !parsed.pixQrCode && !parsed.pixUrl) {
      console.error("[BrutalCash] Resposta sem dados de PIX");
...
if (!isVercel) {
  app.listen(PORT, "127.0.0.1", () => {
    ...
    if (UTMIFY_TOKEN) {
      console.log("[Utmify] Ativo — pedidos Pix serão enviados (waiting_payment + paid)");
